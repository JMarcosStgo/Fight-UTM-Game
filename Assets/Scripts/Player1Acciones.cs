...
public class Player1Acciones : MonoBehaviour
{
    ...
    void Update()
    {
        if (!fin)
        {
            if (!animacion)
            {
                ...
                {
                    FindObjectOfType<Player2Acciones>().OnAtaque.AddListener(RecibirGolpe);
                }

                FindObjectOfType<Player2Acciones>().pierde.AddListener(Win);
